...
async function checkAuthorization(req: Request, res: Response): Promise<any> {
    ...
    const sessions = (await db.query("SELECT userid, token FROM sessions WHERE token = $1 AND isValid = TRUE", [token])).rows;
    ...
    db.query("UPDATE users SET lastUsed = CURRENT_TIMESTAMP WHERE userid = $1", [Number(sessions[0].userid)])
