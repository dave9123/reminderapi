...
export default async function modifyReminder(userid: string, reminderId: string, data: ReminderData) {
    if (!await doesUserExist(userid)) {
        ...
        await db.query("UPDATE reminders SET title = $1, description = $2, time = $3 WHERE userid = $4 AND reminderid = $5", [ data.title, data.description, data.time, userid, reminderId ]);
    }
}